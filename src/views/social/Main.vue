          ...
          <div v-loading="followers.loading">
            <div v-if="followers.list.length">
              <el-row style="min-height: 200px; ">
                <el-col v-for="(item,index) in followers.list" :key="'followers'+index" :span="8" style="padding: 10px">
                  <el-card shadow="hover" style="font-size: 13px;color: #606266;line-height: 20px">
                    <i class="el-icon-star-off"></i>&emsp;
                    <a style=" text-decoration:none;cursor:pointer"
                       @click="$router.push(`/user/social/details/${item.name}`)">{{ item.name }}</a> <br>
                    <i class="el-icon-message"></i>&emsp;
                    <a :href="item.htmlUrl" style=" text-decoration:none;cursor:pointer" target="_blank">TA的主页</a> <br>
                    <img :src="item.avatarUrl" style="width: 100%;border-radius:5px;margin-top: 5px">
                  </el-card>
                </el-col>
              </el-row>
              <div style="text-align: center;margin-top: 10px">
                <el-pagination :current-page.sync="followers.query.page"
            ...
            <div v-else style="min-height: 300px;margin-bottom: 20px;padding: 20px 0px 20px 0px;text-align: center">
              <font style="font-size: 30px;color:#dddddd "> <b>(￢_￢) 没有一个粉丝</b> </font>
            </div>
          </div>
        </el-tab-pane>
        <el-tab-pane :label="'关注 '+followingTotal" name="following" style="padding: 5px">
          <div v-loading="following.loading">
            <div v-if="following.list.length">
              <el-row style="min-height: 200px; ">
                <el-col v-for="(item,index) in following.list" :key="'following'+index" :span="8" style="padding: 10px">
                  <el-card shadow="hover" style="font-size: 13px;color: #606266;line-height: 20px">
                    <i class="el-icon-star-off"></i>&emsp;
                    <a style=" text-decoration:none;cursor:pointer"
                       @click="$router.push(`/user/social/details/${item.name}`)">{{ item.name }}</a> <br>
              ...
              <div style="text-align: center;margin-top: 10px">
                <el-pagination :current-page.sync="following.query.page"
            ...
            <div v-else style="min-height: 300px;margin-bottom: 20px;padding: 20px 0px 20px 0px;text-align: center">
              <font style="font-size: 30px;color:#dddddd "> <b>(￢_￢) 还没有关注一个人</b> </font>
            </div>
          </div>
        </el-tab-pane>
      </el-tabs>
