<template>
  <div :style="'background: #f8f8f8;min-height: '+windowSize.height+'px'">
    ...
    <router-link v-for="(item,index) in projects"
                 :key="'p'+index"
                 :to="`/mobile/user/project/details/${item.name}`"
                 style=" text-decoration:none;cursor:pointer">
      <van-panel :desc="'更新时间 '+item.updateTime" :title="item.name" style="margin-bottom: 10px">
        ...
        <div style="padding: 0px 15px 5px 15px;color: #303133;">
          <van-row>
            ...
            <van-col span="12" style="text-align: right">
              <van-tag v-if="item.license" plain type="danger">{{ item.license }}</van-tag>
